...
const LeaveSummaryCard: React.FC<LeaveSummaryCardProps> = ({ title, value, total, color }) => {
  ...
  const safeTotal = isNaN(total) || !isFinite(total) || total <= 0 ? 1 : total;
      ...
      <CardContent className="p-4">
        <div className="flex flex-col">
          ...
          <div className="flex items-end mt-2">
            <span className={`text-2xl font-bold ${color}`}>{safeValue}</span>
            <span className="text-sm text-gray-500 ml-1">/ {safeTotal}</span>
          </div>
        </div>
      </CardContent>
    </Card>
...
const LeaveSummaryCards: React.FC<LeaveSummaryCardsProps> = ({
  ...
  // Utiliser des valeurs par défaut sûres pour éviter les problèmes de rendu
  const safeTotalDays = isNaN(totalDays) ? 0 : totalDays;
  const safePaidRemaining = isNaN(paidLeavesRemaining) ? 0 : paidLeavesRemaining;
  const safePaidTotal = isNaN(paidLeavesTotal) || paidLeavesTotal <= 0 ? 1 : paidLeavesTotal;
  const safeRttRemaining = isNaN(rttRemaining) ? 0 : rttRemaining;
  const safeRttTotal = isNaN(rttTotal) || rttTotal <= 0 ? 1 : rttTotal;
