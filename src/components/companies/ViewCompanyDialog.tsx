...
const ViewCompanyDialog = ({ companyId, open, onOpenChange }: ViewCompanyDialogProps) => {
  ...
  useEffect(() => {
    // Réinitialiser l'état à la fermeture du dialogue
    if (!open) {
      setCompany(null);
    ...
    // Seulement charger les données si le dialogue est ouvert et que nous n'avons pas encore tenté de charger
    const fetchCompany = async () => {
      if (!companyId || !open || fetchAttempted) return;
    ...
    fetchCompany();
  }, [companyId, open, getById, fetchAttempted]);
  ...
  if (error || (!company && fetchAttempted)) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-xl">
          <div className="flex flex-col items-center justify-center p-8">
